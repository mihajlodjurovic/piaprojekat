/**
 * ============================================
 *  SEED SKRIPTA - SportSphere Hub
 * ============================================
 * 
 * Pokretanje:  npm run seed   (ili: node seed.js)
 * 
 * Ova skripta:
 *  1. Briše sve postojeće podatke iz baze
 *  2. Ubacuje demo podatke (sportove, korisnike, objekte, terene, itd.)
 *  3. Lozinke se hešuju bcrypt-om pre čuvanja u bazi
 *
 * ⚠️  ADMIN NALOG (pre-registrovan, odmah aktivan):
 *     Korisničko ime: admin
 *     Lozinka:        Admin123!
 *
 * 📋  Lozinka za SVE demo korisnike:  Admin123!
 *     (8-12 karaktera, počinje velikim slovom, sadrži broj i specijalni karakter)
 * ============================================
 */
...
// Lozinka koja zadovoljava specifikaciju:
//   - 8-12 karaktera ✓ (9)
//   - počinje slovom ✓ (A)
//   - bar jedno veliko slovo ✓ (A)
//   - bar jedan broj ✓ (123)
...
async function seed() {
  ...
  console.log(`  ✅ ${Object.keys(sports.insertedIds).length} sportova kreirano`);
  ...
  const users = await db.collection('users').insertMany([
    // ═══════════════════════════════════════════
    // ADMIN (pre-registrovan na nivou sistema)
    // ═══════════════════════════════════════════
    {
      username: 'admin',
      email: 'admin@sportsphere.com',
      password: hashedPassword,            // <-- HEŠOVANA lozinka
      role: 'admin',
      firstName: 'Admin',
      lastName: 'Sistema',
      contactPhone: '0600000000',
      profileImage: 'default-avatar.png',
      approvalStatus: 'approved',           // već odobren
      isActive: true,
      favoriteSports: [],
      blockedFacilities: [],
      createdAt: new Date(),
    ...
    // ═══════════════════════════════════════════
    // ZAPOSLENI (employee) - 2
    // ═══════════════════════════════════════════
    {
      username: 'zaposleni1',
      email: 'emp1@sportsphere.com',
      password: hashedPassword,
      role: 'employee',
      firstName: 'Marko',
      lastName: 'Marković',
      contactPhone: '0611111111',
      facilityName: 'Sportski Centar Beograd',
      facilityAddress: 'Bulevar oslobođenja 100, Beograd',
    ...
    {
      username: 'zaposleni2',
      email: 'emp2@sportsphere.com',
      password: hashedPassword,
      role: 'employee',
      firstName: 'Jelena',
      lastName: 'Jovanović',
      contactPhone: '0622222222',
      facilityName: 'Sportski Centar Novi Sad',
      facilityAddress: 'Bulevar Evrope 50, Novi Sad',
    ...
    {
      username: 'sportista3',
      email: 'ath3@sportsphere.com',
      password: hashedPassword,
      role: 'athlete',
      firstName: 'Nikola',
      lastName: 'Nikolić',
      contactPhone: '0655555555',
  ...
  console.log(`  ✅ ${Object.keys(users.insertedIds).length} korisnika kreirano`);
  ...
  const facilities = await db.collection('sport_facilities').insertMany([
    {
      ...
      managers: [emp1Id],
      companyName: 'Sportski Centar Beograd',
      courts: [
      ...
      mainImage: 'default-facility.jpg',
      galleryImages: [],
      likes: 45,
      dislikes: 3,
      likedBy: [],
    ...
    },
    {
      ...
      city: 'Novi Sad',
      address: 'Bulevar Evrope 50',
      description: 'Moderni sportski centar u Novom Sadu.',
      managers: [emp2Id],
      companyName: 'Sportski Centar Novi Sad',
      courts: [
  ...
  console.log(`  ✅ 2 objekta kreirana`);
  ...
  console.log(`  ✅ ${Object.keys(trainers.insertedIds).length} trenera kreirano`);
  ...
  console.log(`  ✅ ${eqIds.length} artikala opreme kreirano`);
  ...
  console.log('  ✅ 3 promocije kreirane');
  ...
  console.log('  ✅ 4 rezervacije kreirane');
  ...
  console.log('  ✅ 3 recenzije kreirane');
  ...
  await db.collection('trainings').insertMany([
    ...
    { athlete: ath3Id, trainer: trainers.insertedIds[1], facility: fac1Id, courtId: fac1Courts[4]._id, sport: 'Košarka', date: new Date(today.getTime() + 6 * 24 * 60 * 60 * 1000), startTime: '14:00', endTime: '16:00', price: 5000, status: 'scheduled', attendanceStatus: 'pending', createdAt: new Date(), updatedAt: new Date() }
  ]);
  console.log('  ✅ 2 treninga kreirana');
  ...
  await db.collection('orders').insertOne({
    user: ath1Id,
    items: [{ equipment: eqIds[0], name: 'Teniski reket Pro', price: 8500, quantity: 1 }],
    totalPrice: 8500,
    facility: fac1Id,
    status: 'ordered',
    createdAt: new Date(),
  ...
  console.log('  ✅ 1 porudžbina kreirana');
  ...
  await db.collection('teammate_posts').insertOne({
    ...
    facility: fac1Id,
    missingPlayers: 3,
    description: 'Tražim 3 igrača za basket. Svi nivoi dobrodošli!',
    joinRequests: [],
    approvedPlayers: [],
    isActive: true,
    isComplete: false,
  ...
  console.log('  ✅ 1 oglas za saigrače kreiran');
  ...
  console.log('│  🏢 Zaposleni│   zaposleni1 (BG)        │   Admin123!       │');
  console.log('│  🏢 Zaposleni│   zaposleni2 (NS)        │   Admin123!       │');
  console.log('└──────────────┴──────────────────────────┴───────────────────┘');

  console.log('\n💡 Lozinka Admin123! zadovoljava sve zahteve iz specifikacije:');
  console.log('   • 8-12 karaktera ✓ (9 karaktera)');
  console.log('   • Počinje slovom ✓');
  console.log('   • Bar jedno veliko slovo ✓');
  console.log('   • Bar jedan broj ✓');
  console.log('   • Bar jedan specijalni karakter ✓');
  console.log('   • Hešovana bcrypt-om u bazi ✓\n');
